...
import { PrismaClient, BatchJobStatus, BatchJobType } from '@/generated/client';
import {
  IBatchJobRepository,
  BatchJob,
  BatchJobData,
...
export class PrismaBatchJobRepository implements IBatchJobRepository {
  ...
  async update(job: BatchJob): Promise<BatchJob> {
    ...
    const updated = await this.prisma.batchJob.update({
      ...
      data: {
        ...
        started_at: data.startedAt,
        completed_at: data.completedAt,
        duration_seconds: data.completedAt && data.startedAt
  ...
  async saveCheckpoints(
    checkpoints: Omit<JobCheckpoint, 'id' | 'createdAt'>[]
  ): Promise<void> {
