...
export class BatchJob {
  ...
  static reconstitute(data: BatchJobData): BatchJob {
    return new BatchJob({
      ...data,
      progress: data.progress instanceof JobProgress
        ? data.progress
        : JobProgress.fromJSON(data.progress as unknown as ReturnType<JobProgress['toJSON']>),
      result: data.result instanceof JobResult
        ? data.result
  ...
  get status(): JobStatus { return this.data.status; }
  get config(): JobConfig { return this.data.config; }
  get progress(): JobProgress { return this.data.progress; }
  get result(): JobResult { return this.data.result; }
  get createdBy(): string { return this.data.createdBy; }
  get organizationId(): string | undefined { return this.data.organizationId; }
  get parentJobId(): string | undefined { return this.data.parentJobId; }
  ...
  get lastHeartbeat(): Date | null { return this.data.lastHeartbeat; }
  get lastCheckpointIndex(): number { return this.data.lastCheckpointIndex; }
  get resumeCount(): number { return this.data.resumeCount; }
  ...
  get isRunning(): boolean {
    return this.data.status === JobStatus.IN_PROGRESS ||
           this.data.status === JobStatus.RESUMING;
  }
   ...
   */
  complete(): void {
    const finalStatus = this.data.progress.failedRecords > 0
  ...
  setTotalRecords(total: number): void {
    ...
    this.data.progress = this.data.progress.withTotal(
      total,
