...
export class AedExportProcessor extends BaseBatchJobProcessor<AedExportConfig> {
  ...
  private async writeCsvFile(fields?: string[]): Promise<void> {
    const allFields = fields && fields.length > 0 ? fields : [
      'id', 'name', 'code', 'status', 'latitude', 'longitude',
      'establishmentType', 'streetName', 'streetNumber', 'postalCode',
      'cityName', 'cityCode', 'districtName', 'createdAt',
    ];
    ...
    for (const record of this.exportedRecords) {
      const values = allFields.map(field => {
        ...
        if (field.startsWith('location.') || ['streetName', 'streetNumber', 'postalCode', 'cityName', 'cityCode', 'districtName'].includes(field)) {
