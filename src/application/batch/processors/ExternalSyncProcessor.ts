...
import { BatchJob } from '@/domain/batch/entities';
import { IDataSourceAdapter } from '@/domain/import/ports/IDataSourceAdapter';
...
export class ExternalSyncProcessor extends BaseBatchJobProcessor<ExternalSyncConfig> {
  ...
  private dataSource: ExternalDataSourceConfig | null = null;
  ...
  async initialize(config: ExternalSyncConfig): Promise<ProcessorInitResult> {
    try {
      ...
      const totalRecords = await this.adapter.getRecordCount(
  ...
  async processChunk(context: ProcessorContext): Promise<ProcessChunkResult> {
    const { job, startIndex, chunkSize, timeoutAt, onCheckpoint, onHeartbeat } = context;
  ...
  async preview(
    ...
    const records = await adapter.getPreview(
      dataSource.config as Record<string, unknown>,
      limit
    );

    const totalCount = await adapter.getRecordCount(
      dataSource.config as Record<string, unknown>
    );
